...
# 2. INICIALIZACIÓN DE VARIABLES GLOBALES (Base de datos interna del navegador)
...
# Base de datos de preguntas y respuestas internas para simular el cerebro de Astra al 100% sin depender de llaves externas que se bloqueen
BANCO_PREGUNTAS = {
    "Matemáticas": [
...
# 3. BARRA LATERAL (Panel Gamer de Control de Astra)
with st.sidebar:
    ...
    # Marcador de XP y Niveles interactivo
    ...
    # Cambiar de modo automáticamente al presionar el botón de radio
    if modo_seleccionado != st.session_state.modo:
        st.session_state.modo = modo_seleccionado
        st.session_state.messages = [] # Limpiar chat para el nuevo modo
        st.session_state.pregunta_actual = ""
        st.rerun()
...
if len(st.session_state.messages) == 0:
    if st.session_state.modo == "🧠 Método Aristotélico (Pistas)":
        bienvenida = f"¡Wena! Soy tu tutor de Astra AI. Ponme cualquier ejercicio o duda de **{materia}** que tengas. Ojo: no te daré la respuesta al tiro, te iré dando pistas cortas para que tu cerebro haga el trabajo duro. ¿Qué problema resolvemos?"
    elif st.session_state.modo == "🗣️ AI Oral Exam (Modo Quiz)":
        # Seleccionar primera pregunta del banco
        pregunta = random.choice(BANCO_PREGUNTAS[materia])
        st.session_state.pregunta_actual = pregunta
        bienvenida = f"🚀 ¡Iniciamos el Examen Oral de **{materia}**! Prepárate. Aquí va la primera pregunta:\n\n**{pregunta['p']}**\n\nEscribe tu respuesta abajo (o lo que creas que es) y la evaluamos al tiro."
    else:
        bienvenida = f"📇 ¡赫 Selector de Materiales activado! Escribe el tema específico de **{materia}** que estás viendo en el liceo y te armaré tarjetas de memoria y un resumen pro para salvar la prueba."
    
    st.session_state.messages.append({"role": "astra", "content": bienvenida})

# Mostrar los mensajes almacenados con diseño personalizado
...
# 5. LOGICA INTERACTIVA DE RESPUESTAS (El Cerebro Automatizado)
if user_input := st.chat_input("Escribe tu duda o respuesta aquí..."):
    ...
    with st.spinner("Astra analizando..."):
        ...
        # LÓGICA MODO ARISTOTÉLICO
        if st.session_state.modo == "🧠 Método Aristotélico (Pistas)":
            respuesta_astra = f"¡Buena pregunta! Respecto a lo que planteas sobre **{materia}**, tu enfoque va bien encaminado. Pero para no arruinarte el aprendizaje, analicemos este detalle: ¿qué pasa si aplicas el concepto base primero? Cuéntame qué crees que se debería hacer en ese primer paso y lo revisamos juntos punto por punto. ¡Dale!"
        
        # LÓGICA MODO QUIZ / EXAMEN ORAL
        elif st.session_state.modo == "🗣️ AI Oral Exam (Modo Quiz)":
            ...
            if solucion_correcta in user_input.lower():
                ...
                respuesta_astra = f"¡Excelente! Le achuntaste medio a medio. Te ganaste **+35 XP** por tu nivel de análisis. 🏆\n\nPasemos al siguiente desafío de **{materia}**:\n\n**{nueva_pregunta['p']}**"
            else:
                respuesta_astra = f"Buen intento, pero no va por ahí el concepto exacto. Te doy una pista: se relaciona directamente con la palabra clave que empieza con '{solucion_correcta[0]}'. ¡Piénsala bien y vuelve a responder!"
        
        # LÓGICA MODO FLASHCARDS
        else:
            respuesta_astra = f"✨ ¡Material de repaso completado con éxito para **{materia}**! Aquí tienes tus herramientas:\n\n" \
                              f"### 📇 Flashcards (Tarjetas de Memoria):\n" \
                              f"1. **Pregunta:** ¿Cuál es la regla principal de este concepto? \n   *Respuesta:* Estudiar por partes y conectar las ideas clave.\n" \
                              f"2. **Pregunta:** ¿Qué error común se comete aquí? \n   *Respuesta:* Tratar de memorizar todo de corrido sin entender el porqué.\n\n" \
                              f"### 📜 Resumen Pro Académico:\n" \
                              f"• **Punto 1:** El núcleo del tema que mencionas de {user_input} se basa en dominar las fórmulas básicas.\n" \
                              f"• **Punto 2:** Siempre repasa los errores de las guías anteriores, ahí está el truco para subir las notas en el liceo.\n\n" \
                              f"¿Quieres que profundicemos en alguna de estas flashcards?"

        # Mostrar respuesta de Astra
